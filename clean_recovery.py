...
def shutdown_prompt(success, notifier, symlink_errors, totals):
    ...
    if not has_errors:
        ...
        if ans_merge == "y":
            log("Running recovery_merge.py...")
            notifier.notify("Running recovery merge...")
            merge_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "recovery_merge.py")
            cmd = ["sudo", "python3", merge_script, "--src", RECOVERED_ROOT, "--dst", ACTIVE_ROOT, "--log", os.path.join(RECOVERY_DIR, "recovery_merge.log")]
            try:
                res = subprocess.run(cmd, capture_output=True, text=True)
                for line in res.stdout.splitlines():
                    log(f"  [merge] {line}")
                if res.returncode == 0:
                    log("✔   Recovery Merge completed successfully.")
                    notifier.notify("Recovery merge completed.")
                else:
                    log(f"❌  Recovery Merge failed (exit code {res.returncode}):")
                    for line in res.stderr.splitlines():
                        log(f"  [merge err] {line}")
                    notifier.notify("Recovery merge failed.")
            except Exception as e:
                log(f"❌  Error executing recovery merge: {e}")
